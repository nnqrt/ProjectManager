...
function getScheduleColorStyle(tag) {
  ...
  // Schedule tags
  if (tag.includes('회의')) return 'background: #EFF6FF; color: #1E3A8A; border-left: 3px solid #2563EB;'; // Deep Blue
  if (tag.includes('방문')) return 'background: #FDF4FF; color: #86198F; border-left: 3px solid #D946EF;'; // Fuchsia
  if (tag.includes('행사')) return 'background: #ECFEFF; color: #164E63; border-left: 3px solid #06B6D4;'; // Cyan
  if (tag.includes('언론')) return 'background: #FFFBEB; color: #92400E; border-left: 3px solid #F59E0B;'; // Amber
  if (tag.includes('관리')) return 'background: #F5F3FF; color: #4C1D95; border-left: 3px solid #7C3AED;'; // Violet
  if (tag.includes('지역')) return 'background: #FDF2F8; color: #9D174D; border-left: 3px solid #EC4899;'; // Pink
  if (tag.includes('긴급') || tag.includes('D-DAY')) return 'background: #FEF2F2; color: #991B1B; border-left: 3px solid #DC2626;'; // Red
  if (tag.includes('비공식')) return 'background: #F8FAFC; color: #334155; border-left: 3px solid #94A3B8;'; // Gray
...
function openTaskDetailModal(taskId) {
  ...
  modalBody.innerHTML = `
    ...
    ${task.status === 'APPROVED' ? `
      <div style="background: #ECFDF5; border: 1px solid #10B981; border-radius: 6px; padding: 8px 12px; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
        <span style="font-size: 15px;">✅</span>
        <span style="font-size: 13px; font-weight: 800; color: #065F46;">결재 승인 완료 안건 (최종결재권자: ${task.finalApproverName || '의원/총괄'})</span>
      </div>
    ` : (task.status === 'REJECTED' ? `
      <div style="background: #FEF2F2; border: 1px solid #DC2626; border-radius: 6px; padding: 8px 12px; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
        <span style="font-size: 15px;">❌</span>
        <span style="font-size: 13px; font-weight: 800; color: #991B1B;">반려 처리된 안건</span>
      </div>
    ` : '')}
...
function renderAssigneeView() {
  ...
  container.innerHTML = displayTasks.map(t => {
    ...
    return `
      <div class="card card-clickable ${deadline.isUrgent && !t.isCompleted ? 'deadline-urgent-card' : ''}" style="display: flex; flex-direction: column; justify-content: space-between; border-left: 5px solid ${t.isCompleted ? '#047857' : (isApproved ? '#10B981' : color)};" onclick="openTaskDetailModal('${t.id}')">
        <div>
